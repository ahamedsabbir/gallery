...
@section('content')
	...
	<section class="ftco-section">
		<div class="container">
			...
			<div class="row">
			@foreach($stories as $stories_key => $stories_value)
			  <div class="col-md-4 ftco-animate">
				<div class="blog-entry">
				  <a href="{{ asset('/klift/uploads/'.$stories_value->thumb) }}" class="block-20" style="background-image: url('{{ asset('/klift/uploads/'.$stories_value->thumb) }}');">
				  </a>
				  <div class="text d-flex py-4">
					<div class="meta mb-3">
					  <div><a href="">{{ naturalDate($stories_value->created_at) }}</a></div>
					</div>
					<div class="desc pl-3">
						<h3 class="heading"><a href="{{ url('story/'.$stories_value->id) }}">{{ $stories_value->title }}</a></h3>
					  </div>
				  </div>
				</div>
			  </div>
